...
import { UpdateEnsembleDto } from 'src/ensembles/dto/update-ensemble.dto';
...
describe('EnsemblesController (e2e)', () => {
    ...
    describe('/ensembles/:id/users (POST)', () => {
      ...
      beforeEach(async () => {
        // Create the ensemble
        const ensemblePayload = { name: 'Link Test Ensemble', description: 'Test' };
      ...
      it('should link a user to the ensemble', async () => {
        ...
        const userLinkPayload: linkUserToEnsembleDto = { username: 'testuser1' };
        const response = await request(app.getHttpServer())
          .post(`/ensembles/${ensembleId}/users`)
          .send(userLinkPayload)
          .expect(201);
        ...
        expect(response.body.users).toContainEqual(
          expect.objectContaining({ username: userLinkPayload.username }),
        );
      });
    });
    ...
    describe('/ensembles/:id (DELETE)', () => {
      it('should delete an ensemble by ID', async () => {
        ...
        const payload = { name: 'Delete Test Ensemble', description: 'To be deleted' };
      ...
      it('should return 404 if the ensemble does not exist', async () => {
        ...
        expect(response.body.message).toBe(`Ensemble with id ${invalidEnsembleId} not found`);
      });
    });
    ...
    describe('PATCH /ensembles/:id - Partial Updates', () => {
      ...
      beforeEach(async()=>{
        const ensemble: CreateEnsembleDto = { name: 'original ensemble', description: 'This is a original ensemble' };
        const response = await request(app.getHttpServer())
         .post('/ensembles')
         .send(ensemble)
         .expect(201);
      ...
      it ('should update only the description of the ensemble',async()=>{
        ...
        expect(response.body.description).toBe('This is updated ensemble description');
        expect(response.body.users).toEqual([]);
      });
      ...
      it('should return 404 when trying to update a non-existent ensemble', async () => {
        ...
        expect(response.body.message).toBe('Ensemble with ID 63c15d7b8d5a5a001c8b4567 not found');
      });
    });
});